...
import { getPopularMoviesApi } from "@/network/apis";
...
export default function PopularNow() {
    const [popularNow, setPopularNow] = useState<Movie[]>([]);
    ...
    const fetchMovies = async (page: number) => {
        ...
        try {
            ...
            setPopularNow((prev) => [...prev, ...data.results]);
            setTotalPages(data.total_pages);
            setCurrentPage(page);
        } catch (error) {
            console.error("Failed to fetch popular now movies:", error);
        } finally {
            setIsFetching(false);
        }
    };
